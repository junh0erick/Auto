...
%    CTRL mode:
%      's' -> stop                      -> PSoC: 'K'
%      'f' -> live ref update (4B raw)  -> PSoC: 'K'  (no echo-confirm)
%  Telemetry: 16 B/frame  [y2_f32  u1_f32  y1_f32  u2_f32]
%
%  Modes: 0=TF  1=SS-Pred-NoI  2=SS-Act-NoI  3=SS-Pred-I  4=SS-Act-I
%         5=OpenLoop  6=Off
%
%  Coeff layout (25 single-precision floats, 1-indexed MATLAB):
%    TF:  c(1:6)=b  c(7:12)=a  c(15)=N_divisor  [c(16) only for plant 0: Fs_hz]
%    SS:  c(1:4)=A(2x2 row-maj) c(5:6)=B c(7:8)=C c(9)=D
%         c(10:11)=L c(12:13)=K c(14)=Kx c(15)=N_divisor [c(16) plant 0: Fs_hz]

%% ── Constants ────────────────────────────────────────────────────────────────
FRAME_SZ      = 16;     % bytes per telemetry frame
STEP_TMO      = 0.8;    % s per handshake step
MAX_RETRIES   = 50;
MAX_PTS       = 500000;
...
%% ── Figure ───────────────────────────────────────────────────────────────────
FIG_W = 1500; FIG_H = 960;
fig = uifigure('Name','Péndulo Invertido — Control GUI v2', ...
               'Position',[20 20 FIG_W FIG_H]);
fig.CloseRequestFcn = @onClose;

%% ── Axes (left side, 4-plot initial) ────────────────────────────────────────
AX_X = 8; AX_W = 892;
ax_u1 = uiaxes(fig,'Position',[AX_X 719 AX_W 233]);
ax_y1 = uiaxes(fig,'Position',[AX_X 481 AX_W 233]);
ax_u2 = uiaxes(fig,'Position',[AX_X 243 AX_W 233]);
ax_y2 = uiaxes(fig,'Position',[AX_X   5 AX_W 233]);
...
hold(ax_y1,'on'); hY1=plot  (ax_y1,nan,nan,'b-','LineWidth',1,'DisplayName','ω meas');hold(ax_y1,'off'); legend(ax_y1,'show');
hold(ax_u2,'on'); hU2=stairs(ax_u2,nan,nan,'r-','LineWidth',1,'DisplayName','u₂');    hold(ax_u2,'off'); legend(ax_u2,'show');
hold(ax_y2,'on'); hY2=plot  (ax_y2,nan,nan,'r-','LineWidth',1,'DisplayName','θ meas');hold(ax_y2,'off'); legend(ax_y2,'show');

%% ── Right panel layout ───────────────────────────────────────────────────────
%  FIG_H=960:  conn@920h35 | plant0@595h320 | plant1@270h320
%              ctrl@190h75 | data@106h79    | log@5h96
RX = 907; RW = 586;
PLANT_H = 320;
PLANT_Y = [595, 270];

% ─ 1. Connection ──────────────────────────────────────────────────────────────
pConn = uipanel(fig,'Title','Conexión','Position',[RX 920 RW 35]);
...
lblStat = uilabel(pConn,'Text','Desconectado','Position',[368 5 100 22]);
btnSave = uibutton(pConn,'Text','Guardar','Position',[472 4 54 24],'ButtonPushedFcn',@onSaveSession);
btnLoad = uibutton(pConn,'Text','Cargar', 'Position',[530 4 54 24],'ButtonPushedFcn',@onLoadSession);

% ─ 2 & 3. Plant panels ────────────────────────────────────────────────────────
FW = 58;   % SS editfield width (used by ssEdt helper)
PLANT_LBL = {'Planta 0 — Motor/ω  (inner)','Planta 1 — Péndulo/θ  (outer)'};
ddMode   = gobjects(2,1); ddObs  = gobjects(2,1); cbInt  = gobjects(2,1);
edtN     = gobjects(2,1); edtFs  = gobjects(2,1);
btnSM    = gobjects(2,1); btnSC  = gobjects(2,1);
pTF      = gobjects(2,1); pSS    = gobjects(2,1);
tfTable  = gobjects(2,1); edtTFOrd = gobjects(2,1);
ssFields = cell(2,1);

for pp = 1:2
    pPlant = uipanel(fig,'Title',PLANT_LBL{pp},...
        'Position',[RX PLANT_Y(pp) RW PLANT_H]);

    % ── Mode / Obs / Integrator row ──────────────────────────────────────────
    y_mode = PLANT_H - 42;   % 278
    ...
    % ── N/M row  (Fs only for plant 0) ──────────────────────────────────────
    y_nfs = y_mode - 26;     % 252
    if pp == 1
        uilabel(pPlant,'Text','M:','Position',[6 y_nfs 20 22]);
    else
    ...
    if pp == 1
        ...
        % Plant 1 runs at Fs_inner/N — no independent Fs needed
        uilabel(pPlant,'Text','(N = divisor de Fs_inner)','Position',[104 y_nfs 200 22],...
            'FontColor',[0.5 0.5 0.5]);
        edtFs(pp) = uieditfield(pPlant,'numeric','Value',0,'Limits',[0 10000],...
            'Visible','off','Position',[158 y_nfs 80 22]);
    end

    % ── Buttons ──────────────────────────────────────────────────────────────
    y_btn = y_nfs - 28;      % 224
    btnSM(pp) = uibutton(pPlant,'Text',sprintf('Enviar Modo (%s)',PLANT_CMD_MODE{pp}),...
        'Position',[6 y_btn 148 26],'ButtonPushedFcn',@(~,~) onSendMode(pp));
    btnSC(pp) = uibutton(pPlant,'Text',sprintf('Enviar Coefs (%s)',PLANT_CMD_COEF{pp}),...
        'Position',[160 y_btn 148 26],'ButtonPushedFcn',@(~,~) onSendCoeffs(pp));

    TF_H = y_btn - 6;        % 218  — height of coeff sub-panels

    % ── TF sub-panel ─────────────────────────────────────────────────────────
    pTF(pp) = uipanel(pPlant,'Title','TF  (b/a, máx orden 5)',...
        'Position',[3 0 RW-10 TF_H]);

    % Table at y=5, height=185 → rows 0-5 fully visible
    tdata = [zeros(6,1), [1;zeros(5,1)]];
    tfTable(pp) = uitable(pTF(pp),'Data',tdata,...
        'ColumnName',{'b','a'},'RowName',{'0','1','2','3','4','5'},...
        'ColumnEditable',[true true],'Position',[6 5 230 185]);
    uilabel(pTF(pp),'Text','Orden (0..5):','Position',[246 TF_H-76 110 18]);
    edtTFOrd(pp) = uieditfield(pTF(pp),'numeric','Value',1,'Limits',[0 5],...
        'RoundFractionalValues','on','Position',[246 TF_H-100 60 22]);

    % ── SS sub-panel ─────────────────────────────────────────────────────────
    pSS(pp) = uipanel(pPlant,'Title','SS  (2 estados: A,B,C,D,L,K,Ki)',...
        'Position',[3 0 RW-10 TF_H]);

    % yR = field y-positions from bottom of pSS (all positive within TF_H=218)
    % PAD_GRP=18 extra gap between column groups for readability
    xA=6; xB=xA+FW+4; xC=xB+FW+18; xD=xC+FW+18;
    yR = [168, 140, 96, 48, 20];

    uilabel(pSS(pp),'Text','A (2×2)','Position',[xA yR(1)+24 76 18],'FontSize',10);
    uilabel(pSS(pp),'Text','B (2×1)','Position',[xC yR(1)+24 70 18],'FontSize',10);
    uilabel(pSS(pp),'Text','C (1×2)','Position',[xD yR(1)+24 70 18],'FontSize',10);
    ...
    uilabel(pSS(pp),'Text','D:','Position',[xA yR(3)+24 20 18]);
    fD = ssEdt(pSS(pp),xA+22,yR(3),'0');

    uilabel(pSS(pp),'Text','L (2×1)','Position',[xA yR(4)+24 70 18],'FontSize',10);
    uilabel(pSS(pp),'Text','K (1×2)','Position',[xB yR(4)+24 70 18],'FontSize',10);
    uilabel(pSS(pp),'Text','Ki:',    'Position',[xC yR(4)+24 24 18],'FontSize',10);
    fL1=ssEdt(pSS(pp),xA,yR(4),'0');  fL2=ssEdt(pSS(pp),xA,yR(5),'0');
    fK1=ssEdt(pSS(pp),xB,yR(4),'0');  fK2=ssEdt(pSS(pp),xB,yR(5),'0');
    fKi= ssEdt(pSS(pp),xC+26,yR(4),'0');

    ssFields{pp} = struct('A11',f11,'A12',f12,'A21',f21,'A22',f22,...
        'B1',fB1,'B2',fB2,'C1',fC1,'C2',fC2,'D',fD,...
        'L1',fL1,'L2',fL2,'K1',fK1,'K2',fK2,'Ki',fKi);
end
% Both plant panels fully created — now safe to call refreshPlantVis

% ─ 4. Control ─────────────────────────────────────────────────────────────────
pCtrl = uipanel(fig,'Title','Control','Position',[RX 190 RW 75]);
uilabel(pCtrl,'Text','Ref:','Position',[6 38 28 22]);
edtRef = uieditfield(pCtrl,'numeric','Value',0,'Limits',[-1e6 1e6],...
    'Position',[36 38 70 22]);
btnApplyRef = uibutton(pCtrl,'Text','Aplicar','Position',[110 36 74 26],...
    'ButtonPushedFcn',@onApplyRef);
btnStart = uibutton(pCtrl,'Text','▶  Start (i)','Position',[188 36 110 26],...
    'BackgroundColor',[0.12 0.62 0.12],'FontColor','w','FontWeight','bold',...
    'ButtonPushedFcn',@onStart);
btnStop  = uibutton(pCtrl,'Text','■  Stop (s)', 'Position',[302 36 100 26],...
    'BackgroundColor',[0.78 0.1 0.1],'FontColor','w','FontWeight','bold',...
    'ButtonPushedFcn',@onStop);
lblRunSt = uilabel(pCtrl,'Text','Detenido','Position',[406 36 130 26],...
    'FontWeight','bold');
...
% ─ 5. Datos ───────────────────────────────────────────────────────────────────
pData = uipanel(fig,'Title','Datos','Position',[RX 106 RW 79]);
btnExp   = uibutton(pData,'Text','Exportar .mat','Position',[  6 10 130 28],...
    'ButtonPushedFcn',@onExport);
btnClear = uibutton(pData,'Text','Borrar datos', 'Position',[142 10 120 28],...
    'ButtonPushedFcn',@onClear);
% U scalers
uilabel(pData,'Text','u1×:','Position',[272 36 28 18]);
edtSU1 = uieditfield(pData,'numeric','Value',1,'Limits',[-1e9 1e9],...
    'Position',[302 32 54 22],'ValueChangedFcn',@updatePlots);
uilabel(pData,'Text','u2×:','Position',[362 36 28 18]);
edtSU2 = uieditfield(pData,'numeric','Value',1,'Limits',[-1e9 1e9],...
    'Position',[392 32 54 22],'ValueChangedFcn',@updatePlots);
% Y scalers
uilabel(pData,'Text','y1×:','Position',[272 10 28 18]);
edtSY1 = uieditfield(pData,'numeric','Value',1,'Limits',[-1e9 1e9],...
    'Position',[302  6 54 22],'ValueChangedFcn',@updatePlots);
uilabel(pData,'Text','y2×:','Position',[362 10 28 18]);
edtSY2 = uieditfield(pData,'numeric','Value',1,'Limits',[-1e9 1e9],...
    'Position',[392  6 54 22],'ValueChangedFcn',@updatePlots);

% ─ 6. Log ─────────────────────────────────────────────────────────────────────
txtLog = uitextarea(fig,'Editable','off','Position',[RX 5 RW 96],'FontSize',9);
txtLog.Value = strings(0,1);

% Initial visibility — all widgets ready, safe to call
refreshPlantVis(1);
refreshPlantVis(2);
...
%% ── UI helpers ───────────────────────────────────────────────────────────────
    ...
    function ef = ssEdt(parent, x, y, val)
        ef = uieditfield(parent,'text','Value',val,'Position',[x y FW 22]);
    end
    ...
    function updateAxesLayout()
        inner_on = ~strcmp(ddMode(1).Value,'Off');
        outer_on = ~strcmp(ddMode(2).Value,'Off');
        if inner_on && outer_on
            ax_u1.Visible='on'; ax_y1.Visible='on';
            ax_u2.Visible='on'; ax_y2.Visible='on';
            ax_u1.Position=[AX_X 719 AX_W 233];
            ax_y1.Position=[AX_X 481 AX_W 233];
            ax_u2.Position=[AX_X 243 AX_W 233];
            ax_y2.Position=[AX_X   5 AX_W 233];
        elseif inner_on
            ax_u1.Visible='on';  ax_y1.Visible='on';
            ax_u2.Visible='off'; ax_y2.Visible='off';
            ax_u1.Position=[AX_X 482 AX_W 472];
            ax_y1.Position=[AX_X   5 AX_W 472];
        elseif outer_on
            ax_u1.Visible='off'; ax_y1.Visible='off';
            ax_u2.Visible='on';  ax_y2.Visible='on';
            ax_u2.Position=[AX_X 482 AX_W 472];
            ax_y2.Position=[AX_X   5 AX_W 472];
        else
            ax_u1.Visible='off'; ax_y1.Visible='off';
            ax_u2.Visible='off'; ax_y2.Visible='off';
        end
    ...
    function updatePlots(~,~)
        inner_on = ~strcmp(ddMode(1).Value,'Off');
        outer_on = ~strcmp(ddMode(2).Value,'Off');
        % Always clear NaN first (handles empty-data case)
        set(hU1,'XData',nan,'YData',nan);
        set(hY1,'XData',nan,'YData',nan);
        set(hU2,'XData',nan,'YData',nan);
        set(hY2,'XData',nan,'YData',nan);
        if isempty(S.nVec), return; end
        n = S.nVec;
        su1=edtSU1.Value; if ~isfinite(su1)||su1==0, su1=1; end
        su2=edtSU2.Value; if ~isfinite(su2)||su2==0, su2=1; end
        sy1=edtSY1.Value; if ~isfinite(sy1)||sy1==0, sy1=1; end
        sy2=edtSY2.Value; if ~isfinite(sy2)||sy2==0, sy2=1; end
        if inner_on
            set(hU1,'XData',n,'YData',S.u1Vec*su1);
            set(hY1,'XData',n,'YData',S.y1Vec*sy1);
        end
        if outer_on
            set(hU2,'XData',n,'YData',S.u2Vec*su2);
            set(hY2,'XData',n,'YData',S.y2Vec*sy2);
        end
    end
...
%% ── UARTP2 High-level ────────────────────────────────────────────────────────
    ...
    function uartp2_sendcoeffs(plant_idx, coeffs25)
        ...
        if numel(payload)~=100, error("coeffs25: esperaba 100 bytes"); end
        ll_flush();
        rsp=ll_cmd_wait(cmd_char);
    ...
    function uartp2_setref_live(val)
        % 'f' command: live inner reference update, no echo-confirm
        write(S.sp,uint8('f'),"uint8");
        write(S.sp,typecast(single(val),'uint8'),"uint8");
        t0=tic;
        while toc(t0)<0.5
            if S.sp.NumBytesAvailable>0
                b=read(S.sp,1,"uint8"); b=b(1);
        ...
        error("uartp2_setref_live: timeout");
    end
...
%% ── Coefficient builders ─────────────────────────────────────────────────────
    function c = make_tf(pp)
        ...
        if iscell(D)
            ...
            for r=1:size(D,1)
                for cc=1:size(D,2)
                    v=str2double(string(D{r,cc})); if ~isfinite(v), v=0; end
                    D2(r,cc)=v;
                end
            end
            D=D2;
        ...
        if pp==1, c(16)=single(edtFs(pp).Value); end  % Fs only for inner
    end
    ...
    function c = make_ss(pp)
        ...
        c(14)=single(ssVal(sf.Ki));
        c(15)=single(round(edtN(pp).Value));
        if pp==1, c(16)=single(edtFs(pp).Value); end
    end
    ...
    function c = build_coeffs(pp)
        mn=ddMode(pp).Value;
        if     strcmp(mn,'TF'),       c=make_tf(pp);
        elseif strcmp(mn,'SS'),        c=make_ss(pp);
        else,                         c=make_ol_off(pp); end
    end
...
%% ── Actions ──────────────────────────────────────────────────────────────────
    ...
    function onSaveSession(~,~)
        ...
        try
            sess=struct();
            for pp=1:2
                sess.(sprintf('mode%d',pp))=ddMode(pp).Value;
                sess.(sprintf('obs%d',pp))=ddObs(pp).Value;
                sess.(sprintf('int%d',pp))=cbInt(pp).Value;
                sess.(sprintf('Ndiv%d',pp))=edtN(pp).Value;
                sess.(sprintf('TFdata%d',pp))=tfTable(pp).Data;
                sess.(sprintf('TFord%d',pp))=edtTFOrd(pp).Value;
                sf=ssFields{pp};
                sess.(sprintf('SS%d',pp))=struct(...
                    'A11',sf.A11.Value,'A12',sf.A12.Value,...
                    'A21',sf.A21.Value,'A22',sf.A22.Value,...
                    'B1',sf.B1.Value,'B2',sf.B2.Value,...
                    'C1',sf.C1.Value,'C2',sf.C2.Value,...
                    'D',sf.D.Value,...
                    'L1',sf.L1.Value,'L2',sf.L2.Value,...
                    'K1',sf.K1.Value,'K2',sf.K2.Value,...
                    'Ki',sf.Ki.Value);
            end
        ...
        catch e, logMsg("Guardar sesión FAIL: "+string(e.message)); end
    end
    ...
    function onLoadSession(~,~)
        ...
        try
            ...
            SS_FNAMES={'A11','A12','A21','A22','B1','B2','C1','C2','D','L1','L2','K1','K2','Ki'};
            for pp=1:2
                fld=sprintf('mode%d',pp);
                if isfield(sess,fld) && any(strcmp(MODE_NAMES,sess.(fld)))
                    ddMode(pp).Value=sess.(fld); end
                fld=sprintf('obs%d',pp);
                if isfield(sess,fld), ddObs(pp).Value=sess.(fld); end
                fld=sprintf('int%d',pp);
                if isfield(sess,fld), cbInt(pp).Value=sess.(fld); end
                fld=sprintf('Ndiv%d',pp);
                if isfield(sess,fld), edtN(pp).Value=sess.(fld); end
                fld=sprintf('TFdata%d',pp);
                if isfield(sess,fld), tfTable(pp).Data=sess.(fld); end
                fld=sprintf('TFord%d',pp);
                if isfield(sess,fld), edtTFOrd(pp).Value=sess.(fld); end
                fld=sprintf('SS%d',pp);
                if isfield(sess,fld)
                    sf=ssFields{pp}; sv=sess.(fld);
                    for fi=1:numel(SS_FNAMES)
                        fn=SS_FNAMES{fi};
                        if isfield(sv,fn), sf.(fn).Value=sv.(fn); end
                    end
                end
            end
            ...
            refreshPlantVis(1); refreshPlantVis(2);
            logMsg("Sesión cargada: "+string(f));
        catch e, logMsg("Cargar sesión FAIL: "+string(e.message)); end
    end
    ...
    function onSendCoeffs(pp)
        ...
        try
            ...
            if pp==1
                logMsg(sprintf("Planta 0: coefs enviados. M=%d  Fs=%.1fHz",round(c(15)),c(16)));
            else
                logMsg(sprintf("Planta 1: coefs enviados. N=%d",round(c(15))));
            end
        catch e
    ...
    function onApplyRef(~,~)
        if ~reqConn(), return; end
        if ~S.streamOn
            logMsg("⚠ Control no activo — use Start para iniciar con nueva ref.");
            return;
        end
        ref_new=edtRef.Value;
        try
            S.inTxn=true;
            uartp2_setref_live(ref_new);
            logMsg(sprintf("Ref interna actualizada: %.4g",ref_new));
        catch e
            logMsg("ApplyRef FAIL: "+string(e.message));
        end
        S.inTxn=false;
    end
    ...
    function onStart(~,~)
        ...
        try
            ...
            for pp=1:2
                ...
                uartp2_sendcoeffs(pp,c);
                if pp==1
                    logMsg(sprintf("Planta 0: modo=%d  M=%d  Fs=%.1fHz",mv,round(c(15)),c(16)));
                else
                    logMsg(sprintf("Planta 1: modo=%d  N=%d",mv,round(c(15))));
                end
            end
            uartp2_start(ref0);
...
%% ── Streaming ────────────────────────────────────────────────────────────────
    ...
    function onStreamTick(~,~)
        ...
        try
            ...
            if nFrames>5
                logMsg(sprintf("⚠ Backlog: %d frames, descartando.",nFrames-1));
            ...
            while floor(numel(S.rxBuf)/FRAME_SZ)>0
                ...
                parseFrame(frame); nProc=nProc+1;
            end
            ...
            if nProc>0
                updatePlots(); updateDataInfo(); drawnow limitrate;
            end
        catch e
            S.rxBuf=uint8([]);
    ...
    function parseFrame(frame)
        % PSoC sends [y2, u1, y1, u2] — matches observed hardware frame order
        y2=double(typecast(frame(1:4),  'single'));
        u1=double(typecast(frame(5:8),  'single'));
        y1=double(typecast(frame(9:12), 'single'));
        u2=double(typecast(frame(13:16),'single'));
        if ~isfinite(u1),u1=0;end; if ~isfinite(y1),y1=0;end
        if ~isfinite(u2),u2=0;end; if ~isfinite(y2),y2=0;end
        n=S.framesTotal+1;
        S.nVec(end+1,1)=n;
        S.u1Vec(end+1,1)=u1; S.y1Vec(end+1,1)=y1;
        S.u2Vec(end+1,1)=u2; S.y2Vec(end+1,1)=y2;
        S.framesTotal=n;
...
%% ── Data management ──────────────────────────────────────────────────────────
    function onExport(~,~)
        ...
        try
            data=struct();
            % Telemetry signals
            data.n=S.nVec; data.u1=S.u1Vec; data.y1=S.y1Vec;
            data.u2=S.u2Vec; data.y2=S.y2Vec;
            % Metadata
            data.fecha         = datestr(now,'yyyy-mm-dd HH:MM:SS');
            data.Fs_inner      = edtFs(1).Value;
            data.M_plant0      = edtN(1).Value;   % Plant 0 timer divisor M
            data.N_plant1      = edtN(2).Value;   % Plant 1 outer-loop decimation N
            data.Fs_outer      = data.Fs_inner / max(1, data.N_plant1);
            data.scaler_u1     = edtSU1.Value;
            data.scaler_y1     = edtSY1.Value;
            data.scaler_u2     = edtSU2.Value;
            data.scaler_y2     = edtSY2.Value;
            for pp=1:2
                mv=get_mode_val(pp);
                data.(sprintf('mode_name%d',pp-1)) = ddMode(pp).Value;
                data.(sprintf('obs%d',pp-1))        = ddObs(pp).Value;
                data.(sprintf('integrador%d',pp-1)) = cbInt(pp).Value;
                data.(sprintf('mode_val%d',pp-1))   = mv;
                data.(sprintf('coeffs%d',pp-1))     = double(build_coeffs(pp));
            end
