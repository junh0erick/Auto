...
#ifdef PWM_Motor_PWMUDB_genblk1_ctrlreg__REMOVED
    #error PWM_v3_30 detected with a constant 0 for the enable or \
         constant 1 for reset. This will prevent the component from operating.
#endif /* PWM_Motor_PWMUDB_genblk1_ctrlreg__REMOVED */
...
void PWM_Motor_Init(void) 
{
        ...
        ctrl = PWM_Motor_CONTROL3 & ((uint8 )(~PWM_Motor_CTRL_CMPMODE1_MASK));
        PWM_Motor_CONTROL3 = ctrl | PWM_Motor_DEFAULT_COMPARE1_MODE;

         /* Clear and Set SYNCTC and SYNCCMP bits of RT1 register */
        PWM_Motor_RT1 &= ((uint8)(~PWM_Motor_RT1_MASK));
        PWM_Motor_RT1 |= PWM_Motor_SYNC;

        /*Enable DSI Sync all all inputs of the PWM*/
        PWM_Motor_RT1 &= ((uint8)(~PWM_Motor_SYNCDSI_MASK));
        PWM_Motor_RT1 |= PWM_Motor_SYNCDSI_EN;

    #elif (PWM_Motor_UseControl)
        /* Set the default compare mode defined in the parameter */
        ctrl = PWM_Motor_CONTROL & ((uint8)(~PWM_Motor_CTRL_CMPMODE2_MASK)) &
                ((uint8)(~PWM_Motor_CTRL_CMPMODE1_MASK));
...
{
    #if (PWM_Motor_UseControl || PWM_Motor_UsingFixedFunction)
        PWM_Motor_CONTROL &= ((uint8)(~PWM_Motor_CTRL_ENABLE));
    #endif /* (PWM_Motor_UseControl || PWM_Motor_UsingFixedFunction) */

    /* Globally disable the Fixed Function Block chosen */
    #if (PWM_Motor_UsingFixedFunction)
        PWM_Motor_GLOBAL_ENABLE &= ((uint8)(~PWM_Motor_BLOCK_EN_MASK));
        PWM_Motor_GLOBAL_STBY_ENABLE &= ((uint8)(~PWM_Motor_BLOCK_STBY_EN_MASK));
    #endif /* (PWM_Motor_UsingFixedFunction) */
}
        ...
        *******************************************************************************/
        void PWM_Motor_SetCompareMode(uint8 comparemode) 
        {
                ...
                #if(0 != PWM_Motor_CTRL_CMPMODE1_SHIFT)
                    uint8 comparemodemasked = ((uint8)((uint8)comparemode << PWM_Motor_CTRL_CMPMODE1_SHIFT));
                #else
                    uint8 comparemodemasked = comparemode;
                #endif /* (0 != PWM_Motor_CTRL_CMPMODE1_SHIFT) */

                PWM_Motor_CONTROL3 &= ((uint8)(~PWM_Motor_CTRL_CMPMODE1_MASK)); /*Clear Existing Data */
                PWM_Motor_CONTROL3 |= comparemodemasked;

            #elif (PWM_Motor_UseControl)

                #if(0 != PWM_Motor_CTRL_CMPMODE1_SHIFT)
                    uint8 comparemode1masked = ((uint8)((uint8)comparemode << PWM_Motor_CTRL_CMPMODE1_SHIFT)) &
                                                PWM_Motor_CTRL_CMPMODE1_MASK;
                #else
                    uint8 comparemode1masked = comparemode & PWM_Motor_CTRL_CMPMODE1_MASK;
                #endif /* (0 != PWM_Motor_CTRL_CMPMODE1_SHIFT) */

                #if(0 != PWM_Motor_CTRL_CMPMODE2_SHIFT)
                    uint8 comparemode2masked = ((uint8)((uint8)comparemode << PWM_Motor_CTRL_CMPMODE2_SHIFT)) &
                                               PWM_Motor_CTRL_CMPMODE2_MASK;
                #else
                    uint8 comparemode2masked = comparemode & PWM_Motor_CTRL_CMPMODE2_MASK;
                #endif /* (0 != PWM_Motor_CTRL_CMPMODE2_SHIFT) */

                /*Clear existing mode */
                PWM_Motor_CONTROL &= ((uint8)(~(PWM_Motor_CTRL_CMPMODE1_MASK |
                                            PWM_Motor_CTRL_CMPMODE2_MASK)));
        ...
        *******************************************************************************/
        void PWM_Motor_SetCompareMode1(uint8 comparemode) 
        {
                ...
                uint8 comparemodemasked = ((uint8)((uint8)comparemode << PWM_Motor_CTRL_CMPMODE1_SHIFT)) &
                                           PWM_Motor_CTRL_CMPMODE1_MASK;
            #else
                uint8 comparemodemasked = comparemode & PWM_Motor_CTRL_CMPMODE1_MASK;
            #endif /* (0 != PWM_Motor_CTRL_CMPMODE1_SHIFT) */

            #if (PWM_Motor_UseControl)
                PWM_Motor_CONTROL &= ((uint8)(~PWM_Motor_CTRL_CMPMODE1_MASK)); /*Clear existing mode */
    ...
    *******************************************************************************/
    void PWM_Motor_SetCompareMode2(uint8 comparemode) 
    {
        ...
        #if(0 != PWM_Motor_CTRL_CMPMODE2_SHIFT)
            uint8 comparemodemasked = ((uint8)((uint8)comparemode << PWM_Motor_CTRL_CMPMODE2_SHIFT)) &
                                                 PWM_Motor_CTRL_CMPMODE2_MASK;
        #else
            uint8 comparemodemasked = comparemode & PWM_Motor_CTRL_CMPMODE2_MASK;
        #endif /* (0 != PWM_Motor_CTRL_CMPMODE2_SHIFT) */

        #if (PWM_Motor_UseControl)
            PWM_Motor_CONTROL &= ((uint8)(~PWM_Motor_CTRL_CMPMODE2_MASK)); /*Clear existing mode */
            PWM_Motor_CONTROL |= comparemodemasked;
        #endif /* (PWM_Motor_UseControl) */
    }
    ...
    *******************************************************************************/
    uint16 PWM_Motor_ReadCounter(void) 
    {
        ...
        /* It is up to the user code to make sure there isn't already captured data in the FIFO */
          (void)CY_GET_REG8(PWM_Motor_COUNTERCAP_LSB_PTR_8BIT);
        ...
        *******************************************************************************/
        ...
        {
            while(0u != (PWM_Motor_ReadStatusRegister() & PWM_Motor_STATUS_FIFONEMPTY))
            {
                (void)PWM_Motor_ReadCapture();
            }
        }
...
void PWM_Motor_WritePeriod(uint16 period) 
{
    ...
    *******************************************************************************/
                                       ...
                                       
    {
            ...
            #if(PWM_Motor_UsingFixedFunction)
                CY_SET_REG16(PWM_Motor_COMPARE2_LSB_PTR, (uint16)(compare + 1u));
            #else
                CY_SET_REG16(PWM_Motor_COMPARE2_LSB_PTR, (compare + 1u));
            #endif /* (PWM_Motor_UsingFixedFunction) */
    ...
    *******************************************************************************/
    void PWM_Motor_WriteDeadTime(uint8 deadtime) 
    {
        /* If using the Dead Band 1-255 mode then just write the register */
            ...
            /* Clear existing data */
            PWM_Motor_DEADBAND_COUNT &= ((uint8)(~PWM_Motor_DEADBAND_COUNT_MASK));

            /* Set new dead time */
            #if(PWM_Motor_DEADBAND_COUNT_SHIFT)
                PWM_Motor_DEADBAND_COUNT |= ((uint8)((uint8)deadtime << PWM_Motor_DEADBAND_COUNT_SHIFT)) &
                                                    PWM_Motor_DEADBAND_COUNT_MASK;
            #else
                PWM_Motor_DEADBAND_COUNT |= deadtime & PWM_Motor_DEADBAND_COUNT_MASK;
            #endif /* (PWM_Motor_DEADBAND_COUNT_SHIFT) */
    ...
    *******************************************************************************/
    uint8 PWM_Motor_ReadDeadTime(void) 
    {
        /* If using the Dead Band 1-255 mode then just read the register */
            ...
            /* Otherwise the data has to be masked and offset */
            #if(PWM_Motor_DEADBAND_COUNT_SHIFT)
                return ((uint8)(((uint8)(PWM_Motor_DEADBAND_COUNT & PWM_Motor_DEADBAND_COUNT_MASK)) >>
    ...
    *******************************************************************************/
    ...
    {
        CY_SET_REG8(PWM_Motor_STATUS_MASK_PTR, interruptMode);
    }


    /*******************************************************************************
    * Function Name: PWM_Motor_ReadStatusRegister
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return (CY_GET_REG8(PWM_Motor_STATUS_PTR));
    }
    ...
    /*******************************************************************************
    * Function Name: PWM_Motor_ReadControlRegister
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    uint8 PWM_Motor_ReadControlRegister(void) 
    {
        ...
        result = CY_GET_REG8(PWM_Motor_CONTROL_PTR);
        return (result);
    }


    /*******************************************************************************
    * Function Name: PWM_Motor_WriteControlRegister
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        CY_SET_REG8(PWM_Motor_CONTROL_PTR, control);
    }
    ...
    *******************************************************************************/
    ...
    {
        #if(PWM_Motor_UsingFixedFunction)
            return ((uint16)CY_GET_REG16(PWM_Motor_COMPARE1_LSB_PTR));
        #else
            return (CY_GET_REG16(PWM_Motor_COMPARE1_LSB_PTR));
        #endif /* (PWM_Motor_UsingFixedFunction) */
    }
    ...
    /*******************************************************************************
    * Function Name: PWM_Motor_ReadCompare1
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: PWM_Motor_ReadCompare2
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return (CY_GET_REG16(PWM_Motor_COMPARE2_LSB_PTR));
    }
...
{
    #if(PWM_Motor_UsingFixedFunction)
        return ((uint16)CY_GET_REG16(PWM_Motor_PERIOD_LSB_PTR));
    #else
        return (CY_GET_REG16(PWM_Motor_PERIOD_LSB_PTR));
    #endif /* (PWM_Motor_UsingFixedFunction) */
}
    ...
    /*******************************************************************************
    * Function Name: PWM_Motor_WriteKillTime
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        CY_SET_REG8(PWM_Motor_KILLMODEMINTIME_PTR, killtime);
    }


    /*******************************************************************************
    * Function Name: PWM_Motor_ReadKillTime
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        return (CY_GET_REG8(PWM_Motor_KILLMODEMINTIME_PTR));
    }
