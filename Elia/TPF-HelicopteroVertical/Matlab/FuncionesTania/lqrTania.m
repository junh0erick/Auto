...
%% =========================
% 3) GANANCIAS: LQR (DISCRETO) + OBSERVADOR (place)
%% =========================

% --- sanity: controlabilidad ---
rc = rank(ctrb(A,B));
if rc < n
    error('El par (A,B) NO es controlable (rank=%d < n=%d). No hay LQR que valga.', rc, n);
end

% --- pesos LQR (Q,R) ---
% LQR regula estados. Como no sabés el "escale" físico de x, una forma razonable
% (y rápida) es penalizar principalmente el error en y usando C'*C.
%

% --- LQR discreto ---
% (Necesita Control System Toolbox. Alternativa con dare abajo.)
[K, P, e_cl] = dlqr(A, B, Q, R);

fprintf('LQR: eig(A-BK) = \n'); disp(e_cl.');

% --- Observador: mantenemos tu diseño por place ---
% (Si querés LQG después, ahí cambiaríamos esto por un Kalman: dlqe)
KeP     = place(A', C', p_obs);
Ke_pred = KeP.';                         % predictor

KeA     = place(A', (C*A)', p_obs);
Ke_act  = KeA.';                         % "actual"

% --- Nbar para seguimiento de referencia (SISO) ---
if size(C,1) ~= 1
    error('Tu C no es SISO (tiene %d salidas). Elegí una fila de C para Nbar.', size(C,1));
end
[~,~,Nbar] = refi(A, B, C, K);
