...
/* ===== opcional: para no saturar TX (si tu UART tiene API) ===== */
#ifndef UARTP_UART_GetTxBufferSize
  #define UARTP_UART_GetTxBufferSize()  UART_GetTxBufferSize()
#endif

/* ===== Telemetry streaming (u,y) =====
   - Llamar 1 vez por sample, al final del control_step().
   - Envia 8 bytes: float u seguido de float y (little-endian típico PSoC).
*/
