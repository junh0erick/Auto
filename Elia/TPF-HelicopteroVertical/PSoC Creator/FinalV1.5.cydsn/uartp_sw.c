...
CY_ISR(UARTP_RxIsr)
{
    while (UARTP_UART_GetRxBufferSize() > 0u)
    {
        ...
        if (b == UARTP_CMD_STOP) {
            g_telem_enable = 0u; /* <- clave: no mezclar stream con respuesta */
...
if (g_init_update_pending)
{
    ...
    g_init_update_pending = 0u;
    g_telem_enable = 1u;     /* <- reanudar stream */
...
void UARTP_Telemetry_Push(float u, float y)
{
    ...
    if (N == 0u) return; /* streaming deshabilitado */
    ...
    /* Opcional: si existe buffer TX, evitamos bloquear/saturar */
#ifdef UARTP_UART_GetTxBufferSize
    /* Si tu UART TX buffer es chico, podés filtrar.
       OJO: el tamaño real depende del componente. Ajustá el umbral si hace falta. */
    if (UARTP_UART_GetTxBufferSize() > 64u) {
        return; /* demasiado lleno, saltamos este sample */
    }
#endif
